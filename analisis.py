def passwd_check(lista):
   ...
   Los siguientes son los criterios para verificar la contraseña:
    ...
    if len(lista) > 12:
        ...
        return_val=False
    if not any(char.isdigit() for char in lista):
        print('la contraseña debe tener al menos un número')
        return_val=False
    if not any(char.isupper() for char in lista):
        print('la contraseña debe tener al menos una letra mayúscula')
        return_val=False
    if not any(char.islower() for char in lista):
        print('la contraseña debe tener al menos una letra minúscula')
        return_val=False
    if not any(char in SpecialSym for char in lista):
